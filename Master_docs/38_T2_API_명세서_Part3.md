...
> **DB v3.6 변경**: DB 설계 v3.6에서 `TB_MOVEMENT_SESSION` 테이블이 신규 추가되었다 (§4.17d 참조). 이 테이블은 이동 세션의 메타데이터(시작/종료 시각, 총 거리, 평균 속도, 이동 수단 등)를 집계 저장한다. 현재 구현은 아래와 같이 `tb_location.movement_session_id` 기반 그룹화 방식이며, `TB_MOVEMENT_SESSION` 테이블 연동은 추후 마이그레이션 예정이다.

이동 세션은 현재 `tb_location.movement_session_id` 컬럼 기준으로 `tb_location` 레코드를 그룹화하여 도출된다. 클라이언트가 세션 시작 시 UUID를 생성하여 `movement_session_id` 필드에 포함해 위치를 저장하면, 서버는 이 UUID를 기반으로 세션을 집계한다.
...
> **DB v3.6 변경**: DB 설계 v3.6에서 `TB_FCM_TOKEN` 테이블이 신규 추가되었다 (§4.29a 참조). 기존 `tb_device_token`은 `TB_FCM_TOKEN`으로 마이그레이션 예정이다. 또한 `TB_NOTIFICATION_PREFERENCE` (알림 세부 설정) 테이블도 추가되었다 (§4.29b 참조).
...
**인증**: 필요 (`authenticate` 미들웨어)
**설명**: 인증된 사용자의 특정 FCM 토큰을 비활성화(`is_active = FALSE`)한다. 레코드는 삭제되지 않으며, `tb_device_token.is_active` (DB v3.6: `TB_FCM_TOKEN.is_active`) 플래그만 변경된다.

**Path Parameters**

| 파라미터 | 타입 | 설명 |
|---------|------|------|
| `tokenId` | string (UUID) | 삭제할 FCM 토큰 ID (`tb_device_token.token_id`, DB v3.6: `TB_FCM_TOKEN.token_id`) |
...
<!-- TODO v3.6: TB_NOTIFICATION_PREFERENCE (알림 세부 설정) 관련 엔드포인트 정의 필요 (DB 설계 문서 v3.6 §4.29b 참조)
     - GET /api/v1/users/me/notification-preferences (알림 설정 조회)
     - PUT /api/v1/users/me/notification-preferences (알림 설정 변경)
     카테고리별 알림 ON/OFF: sos_alert, geofence_alert, attendance_alert, chat_message, guardian_message, system_notice 등
-->
...
> **DB v3.6 참고**: DB 설계 v3.6에서 `TB_COUNTRY_SAFETY` 테이블이 신규 추가되었다 (§4.8a 참조). 국가별 안전 정보(위험 등급, 특별 경보, 유효기간 등)를 관리하며, `TB_COUNTRY.country_code`를 FK로 참조한다. 기존 `TB_MOFA_RISK`의 데이터를 보완하는 역할이다.
...
### 16.11 결제 유형 (payment_type) Enum — v3.6 추가

`TB_PAYMENT.payment_type` 컬럼의 허용 값.
...
> DB 소스: `TB_PAYMENT.payment_type VARCHAR(30)`

---

### 16.12 요금제 유형 (plan_type) Enum — v3.6 추가

`TB_SUBSCRIPTION.plan_type` 컬럼의 허용 값.
...
> DB 소스: `TB_SUBSCRIPTION.plan_type VARCHAR(30)`

---

### 16.13 과금 항목 유형 (item_type) Enum — v3.6 추가

`TB_BILLING_ITEM.item_type` 컬럼의 허용 값.

| 값 | 설명 |
|----|------|
| `guardian_extra` | 추가 가디언 (3~5번째, 1,900원/여행) |
| `premium_feature` | 프리미엄 기능 이용 |
| `data_export` | 데이터 내보내기 |
| `extended_history` | 이력 연장 보관 |

> DB 소스: `TB_BILLING_ITEM.item_type VARCHAR(30)` — 비즈니스 원칙 v5.1 §09 참조

---

### 16.14 환불 정책 (refund_policy) Enum — v3.6 추가

`TB_REFUND_LOG.refund_policy` 컬럼의 허용 값.
...
> DB 소스: `TB_REFUND_LOG.refund_policy VARCHAR(30)` — v3.4.1 추가

---

### 16.15 AI 기능 유형 (ai_feature_type) Enum — v3.6 추가

`TB_AI_USAGE.feature_type` 컬럼의 허용 값.

| 값 | 설명 |
|----|------|
| `risk_analysis` | 위험도 분석 |
| `route_suggestion` | 경로 추천 |
| `emergency_guide` | 긴급 상황 가이드 |
| `translation` | 번역 |
| `chat_summary` | 채팅 요약 |

> DB 소스: `TB_AI_USAGE.feature_type VARCHAR(50)` — Domain N (v3.6 신규)

---

### 16.16 긴급 상황 유형 (emergency_type) Enum — v3.6 추가

`TB_EMERGENCY.emergency_type` 컬럼의 허용 값.
...
> DB 소스: `TB_EMERGENCY.emergency_type VARCHAR(30)` — §4.22c 참조

---

### 16.17 긴급 상황 상태 (emergency_status) Enum — v3.6 추가

`TB_EMERGENCY.status` 컬럼의 허용 값.

| 값 | 설명 |
|----|------|
| `active` | 진행 중인 긴급 상황 |
| `acknowledged` | 확인됨 (수신자가 인지) |
| `resolved` | 해결됨 |
| `false_alarm` | 오발령 |

> DB 소스: `TB_EMERGENCY.status VARCHAR(20)` — §4.22c 참조
...
| 2026-03-05 | v1.1 | DB v3.6 교차검증. 17개 신규 테이블 관련 TODO 마커 추가 (TB_EMERGENCY, TB_CHAT_ROOM, TB_FCM_TOKEN, TB_NOTIFICATION_PREFERENCE, TB_MOVEMENT_SESSION, TB_AI_USAGE, TB_B2B_ORGANIZATION/ADMIN/DASHBOARD_CONFIG, TB_REDEEM_CODE). §16에 7개 Enum 정의 추가 (payment_type, plan_type, item_type, refund_policy, ai_feature_type, emergency_type, emergency_status). tb_device_token → TB_FCM_TOKEN 마이그레이션 주석 추가. |
...
| DB 설계 v3.6 | `Master_docs/07_T2_DB_설계_및_관계_v3_5.md` | 테이블 스키마 원본 (71개 테이블, v3.6 기준) |
...
<!-- TODO v3.6: TB_EMERGENCY / TB_EMERGENCY_RECIPIENT / TB_NO_RESPONSE_EVENT / TB_SAFETY_CHECKIN 관련 상세 엔드포인트 정의 필요 (DB 설계 문서 v3.6 §4.22c~4.22f 참조)
     TB_EMERGENCY: 긴급 상황의 통합 관리 테이블 (TB_SOS_EVENT 상위 개념)
     TB_EMERGENCY_RECIPIENT: 긴급 알림 수신자 추적
     TB_NO_RESPONSE_EVENT: 멤버 무응답 이벤트 기록
     TB_SAFETY_CHECKIN: 안전 체크인 (멤버가 자발적으로 안전 상태 보고)
     - POST /api/v1/safety/checkin (안전 체크인 생성)
     - GET /api/v1/trips/:tripId/safety/checkins (체크인 이력 조회)
     - GET /api/v1/trips/:tripId/no-response-events (무응답 이벤트 이력)
-->
...
> **DB v3.6 참고**: DB 설계 v3.6에서 `TB_EMERGENCY` 테이블이 `TB_SOS_EVENT`의 상위 개념으로 신규 추가되었다 (§4.22c 참조). `TB_SOS_EVENT`는 SOS 전용이며, `TB_EMERGENCY`는 SOS를 포함한 모든 유형의 긴급 상황(사고, 의료, 범죄 등)을 통합 관리한다. 현재 구현은 `TB_EMERGENCY` 테이블 기반이다.
...
> **DB v3.6 참고**: DB 설계 v3.6에서 `TB_CHAT_ROOM` 테이블이 신규 추가되었다 (§4.22g 참조). 채팅방 메타데이터(room_type, 참여자 수, 마지막 메시지 등)를 관리하며, `TB_CHAT_MESSAGE.room_id`가 이 테이블을 FK로 참조한다.
...
<!-- TODO v3.6: TB_REDEEM_CODE (리딤 코드) 관련 엔드포인트 정의 필요 (DB 설계 문서 v3.6 §4.42a 참조)
     - POST /api/v1/payments/redeem (리딤 코드 사용)
     - GET /api/v1/payments/redeem/:code/validate (리딤 코드 유효성 검증)
     - GET /api/v1/payments/redeem/history (리딤 이력 조회)
-->
...
> **DB v3.6 참고**: DB 설계 v3.6에서 B2B 관련 3개 테이블이 신규 추가되었다:
> - `TB_B2B_ORGANIZATION` (§4.42b) — B2B 조직 정보 (학교, 기업 등)
> - `TB_B2B_ADMIN` (§4.42c) — B2B 조직 관리자 계정
> - `TB_B2B_DASHBOARD_CONFIG` (§4.42d) — B2B 대시보드 설정 (표시 항목, 권한 등)
>
> 기존 `TB_B2B_CONTRACT`, `TB_B2B_SCHOOL`, `TB_B2B_INVITE_BATCH`, `TB_B2B_MEMBER_LOG`와 함께 B2B 도메인(L)을 구성한다.
...
<!-- TODO v3.6: TB_AI_USAGE (AI 사용 추적) 관련 엔드포인트 정의 필요 (DB 설계 문서 v3.6 §4.52 참조)
     §N AI (Domain N — 신규 v3.6)
     TB_AI_USAGE: AI 기능별 사용량 추적 (사용자별, 기능별, 토큰 수, 비용 등)
     - POST /api/v1/ai/usage (AI 사용 기록)
     - GET /api/v1/ai/usage?user_id=&feature_type= (AI 사용량 조회)
     - GET /api/v1/ai/usage/summary (AI 사용량 요약 통계)
     ai_feature_type enum: 'risk_analysis' | 'route_suggestion' | 'emergency_guide' | 'translation' | 'chat_summary'
-->
