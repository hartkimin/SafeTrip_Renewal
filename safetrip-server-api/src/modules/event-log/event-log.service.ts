...
export class EventLogService {
    ...
    async find(query: any) {
        const { user_id, group_id, event_type, event_subtype, since, limit, offset } = query;
        const parsedLimit = parseInt(limit || '100', 10);
        const parsedOffset = parseInt(offset || '0', 10);

        try {
            const qb = this.eventLogRepo.createQueryBuilder('e');
            // Explicitly select all columns including select:false ones
            qb.select([
                'e.eventId', 'e.userId', 'e.groupId', 'e.eventType',
                'e.movementSessionId', 'e.eventData', 'e.createdAt',
            ]);
            ...
            console.error('EventLog find error:', error.message);
